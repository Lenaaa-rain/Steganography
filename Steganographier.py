...
def generate_random_filename(length=16):
    """Generates random file names of a specified length, without extensions"""
    ...
    try:
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith(".mp4"):
                ...
                size = get_file_or_folder_size(filepath)  # Get file/folder size
                videos.append({
                    "filename": filename,
                    "duration": formatted_duration,
                    "duration_seconds": duration_seconds or 0,  # If the duration is unknown, it will be 0
                    "size": format_size(size)
                })

        # Sort by Windows display style first
        videos = list(natsorted(videos, key=lambda x: x['filename'], alg=ns.PATH)) 
        
        # If needed, sort by total duration in descending order
        if sort_by_duration:
        ...
        # If there are any errors, an empty list is returned and error messages can be printed for debugging
...
def check_size_and_duration(size, duration_seconds):
    ...
    # Judging by the reference standard
...
class SteganographierGUI:
    '''GUI: Steganography program presentation layer'''
    def __init__(self):
        self.root = TkinterDnD.Tk()
        self.video_folder_path = os.path.join(application_path, "cover_video")  # Define the instance variable The default shell MP4 file storage path
        self.output_option          = "Shell filename" # sets the default value for the output mode
        ...
        self.title = "Steganography Ver.1.2.5 GUI Author: Layer of Forests"
        self.total_file_size = None     # The total size of the steganographed file
        self.password = None            # password
        self.password_modified = False  # Keep track of whether the password has been changed by the user
        self.check_file_size_and_duration_warned = False # Track whether you've been warned about file size-shell duration matching issues
        self.cover_video_options = []         # List of shell mp4 files
        ...
        self.load_config()  # The configuration file is loaded when the program starts
    ...
    def create_widgets(self):
        ...
        try:
            ...
            print("Unable to find icon file, Continue running the program...")
        ...
        def clear_default_password(event):
            if self.password_entry.get() == "If you leave it blank, there is no password":
        ...
        def restore_default_password(event):
            if not self.password_entry.get():
                self.password_entry.insert(0, "If you leave it blank, there is no password")
        ...
        # 工作模式选择
        self.type_option_label = tk.Label(params_frame, text="Working mode:")
        ...
        # 输出选项
        self.output_option_label = tk.Label(params_frame, text="Output name:")
        self.output_option_label.pack(side=tk.LEFT, padx=5, pady=5)
        self.output_option_var = tk.StringVar(value=self.output_option)  # Use the loaded configuration
        self.output_option = tk.OptionMenu(params_frame, self.output_option_var, "Original file name", "The name of the shell file", "Random file name")
        ...
        self.hide_label = tk.Label(self.hide_frame, text="Bulk enter/drag in files/folders that need to be steganomed in this window:") 
        ...
        self.reveal_label = tk.Label(self.reveal_frame, text="In this window, batch input/drag in MP4/MKV files that need to be unsteganized:")
        ...
        video_folder_label = tk.Label(video_folder_frame, text="Shell MP4 file storage:")
        ...
        self.video_folder_button = tk.Button(video_folder_frame, text="Select the folder", command=self.select_video_folder)
        ...
        if self.cover_video_options:
            ...
            # self.output_cover_video_name_mode_var.set('===============Name order pattern===============')  # 默认选择模式
        else:
        ...
        self.log_text.insert(tk.END, "【disclaimer】:\n--This procedure is only used to protect the security of personal information, Do not use it for any illegal or criminal activities--\n--Otherwise, you will bear the consequences, The developer is not responsible for this--\nConsole output goes here...\n\n")
        ...
        self.start_button = tk.Button(button_frame, text="Start executing", command=self.start_thread, width=12, height=2)
        self.start_button.pack(side=tk.LEFT, padx=5)
        
        self.clear_button = tk.Button(button_frame, text="Clear the window", command=self.clear, width=12, height=2)
        self.clear_button.pack(side=tk.LEFT, padx=5)

        self.start_hash_modifier_button = tk.Button(button_frame, text="Hash modifier", command=self.start_hash_modifier, width=12, height=2)
        self.start_hash_modifier_button.pack(side=tk.LEFT, padx=5)
        
        self.start_captcha_generator_button = tk.Button(button_frame, text="Captcha generator", command=self.start_captcha_generator, width=12, height=2)
    ...
    def hide_files_dropped(self, event):
        ...
        for idx, file_path in enumerate(file_paths):
            ...
            if self.type_option_var.get() == "mkv" and size > 2 * 1024 * 1024 * 1024:
                messagebox.showerror("The file size is wrong", "mkv Files cannot be steganomed exceeding 2GB of the Files")
                self.clear()
                return
            ...
            self.log(f"input[{idx+1}]: {os.path.split(file_path)[1]}, File/folder size {format_size(size)}, Predetermined shell duration {format_duration(duration_seconds)}")
    ...
    def check_file_size_and_duration(self, file_path, duration_seconds, idx=0):
        ...
        if not check_size_and_duration(size, duration_seconds) and self.check_file_size_and_duration_warned == False:
            messagebox.showinfo("===========【File steganography is unreasonable reminder】===========", self.get_warning_text(size, duration_seconds, idx))
            self.check_file_size_and_duration_warned = True
    
    # 2.b 给出建议的text文本
    def get_warning_text(self, size, duration_seconds, idx):
        return f'''This pop-up window is only a reminder and is not an error

input [{idx+1}] The volume is {format_size(size)}, Pre-ordered enclosures [{idx+1}] Duration: {format_duration(duration_seconds)}

【Larger in size but shorter in duration】files easily【Arouse suspicion】, suggestion【Volume-splitting compression】Steganography is performed later, Or choose a longer shell video, This warning is only a reminder before the program is restarted 1 times, You can check for problems with other files by referring to the recommended values below. 

Suggested values：
File size		The length of the shell video
--------------------------------
0-200MB		1-3minutes
200-400MB	3-15minutes
400-500MB	15-30minutes
500MB-1GB	30minutes-1hour
1GB-3GB		1hour
3GB-4GB		2hours
>4GB		2hours or more
...
'''
    ...
    def reveal_files_dropped(self, event):
        ...
        for idx, file_path in enumerate(file_paths):
            size = get_file_or_folder_size(file_path)
            self.log(f"input[{idx+1}]: {os.path.split(file_path)[1]} size: {format_size(size)}")
    ...
    def select_video_folder(self):
        ...
        if folder_path:
            ...
            self.cover_video_options +=  ['===============Randomly select the mode===============', 
                                            '===============Duration order mode===============', 
                                            '===============Name order pattern===============']
            if not [item for item in os.listdir(self.video_folder_path) if item.lower().endswith('.mp4')]:
                messagebox.showwarning("Warning", "There are no MP4 files under the folder, Please continue after adding files.")
                self.output_cover_video_name_mode_var.set("No videos found")
    ...
    def check_mkvtools_existence(self):
        ...
        if missing_tools:
            messagebox.showwarning("Warning", "The following tool files are missing, Please continue after adding it in the tools folder: " + ", ".join(missing_tools))
            return False
        return True
    ...
    def start(self):
        ...
        if not any(hide_file_paths) and not any(reveal_file_paths):
            messagebox.showwarning("Warning", "Please enter or drag in the file.")
            # 结束后恢复按钮
            self.start_button.configure(state=tk.NORMAL)
            self.clear_button.configure(state=tk.NORMAL)
            return
        ...
        messagebox.showinfo("Success", "All operations are complete！")
        # 结束后恢复按钮
        self.start_button.configure(state=tk.NORMAL)
        self.clear_button.configure(state=tk.NORMAL)
...
class Steganographier:
    '''The specific functionality of steganography is implemented by this class'''
    def __init__(self, video_folder_path=None, gui_enabled=False, password_file=None):
        ...
        print(f"The shell folder path：{self.video_folder_path}")
    ...
    def initialize_cover_video_files(self):
        """Random selection mode - Initializes the remaining list of available shell files"""
    ...
    def choose_cover_video_file(self, cover_video_CLI=None, 
        ...
        if not cover_video_files:
            raise Exception(f"{video_folder_path} There are no files under the folder, Please continue after adding files.")

        # 2. 否则在cover_video中选择
        if output_cover_video_name_mode == '===============Randomly select the mode===============':
        ...
        elif output_cover_video_name_mode == '===============Duration order mode===============':
        ...
        elif output_cover_video_name_mode == '===============Name order pattern===============':
            ...
            cover_video = cover_video[:cover_video.rfind('.mp4')] + '.mp4'  # 按最后一个.mp4切分, 以去除后续可能存在的时长大小等内容
            print(f'Drop-down menu mode, Video information: {output_cover_video_name_mode}')
        ...
        try:        
            ...
            if self.type_option_var == 'mp4':
                ...
                try:
                    ...
                    self.log(f"An unexpected error occurred while writing an MP4 file: {str(e)}")
                    raise
            ...
            # 4.2. 隐写mkv文件的逻辑
                ...
                try:
                    ...
                    self.log(f"An error occurred while tyganing: {str(cpe)}")
                    ...
                    self.log(f"An unexpected error occurred while executing mkvmerge: {str(e)}")
                    raise

        except Exception as e:
            self.log(f"An unexpected error occurred while steganing: {str(e)}")
            raise
        finally:
            # 5. 删除临时zip文件
    ...
    def get_output_file_path(self, input_file_path=None, 
        ...
        if self.type_option_var == 'mp4':
            ...
            if output_option == 'Original file name':
                if os.path.isdir(input_file_path):
            ...
            elif output_option == 'The name of the shell file':
            ...
            elif output_option == 'Random file name':
                output_file_path = os.path.join(os.path.split(input_file_path)[0], 
        ...
        if self.type_option_var == 'mp4':
            for test_password in password_list:
                try:
                    ...
                    self.log(f"Unable to extract the file {input_file_path}, Use a password {test_password} fail: {str(e)}")

            self.log("All password attempts failed，Unable to extract the file.")
            ...
            def get_attachment_name(input_file_path):
                ...
                try:
                    ...
                    self.log(f"There was an error getting the attachment: {e}")
                    return None
            ...
            def extract_attachment(input_file_path, output_path):
                ...
                try:
                    ...
                    raise Exception(f"An error occurred while extracting the attachment: {e}")
                    ...
                    if attachments_name.endswith('.zip'):
                        ...
                        for test_password in password_list:
                            try:
                                ...
                                self.log(f"Use a password {test_password} Failed to decompress: {e}")

                        self.log("All password attempts failed，Unable to extract the file.")

                    else:
                        self.log(f"Extract attachments {attachments_name} succeed")
                        os.remove(input_file_path)

                except Exception as e:
                    self.log(f"Extract attachments {attachments_name} Error: {e}")
            else:
                self.log("There are no extractable attachments in this MKV file.")

    def run_cli(self, args):
        # self.type_option_var = argparse.Namespace()
        # self.type_option_var.get = lambda: args.type # 模拟.get() 方法

        print(f"Enter the file/folder path: {args.input}")
        print(f"Output file/folder path: {args.output}")
        print(f"password: {args.password}")
        print(f"Output file type: {args.type}")
        print(f"Set the MP4 video path of the shell: {args.cover}")
        print(f"Perform the de-steganography: {args.reveal}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Steganist Ver.1.2.5 CLI Author: The forest is dyed')
    parser.add_argument('-i', '--input', default=None, help='Specify the path to the input file or folder')
    parser.add_argument('-o', '--output', default=None, help='1.Specify the output file name(Include a suffix) [or] 2.Output folder path(The default file name is the original filename+"hidden")')
    parser.add_argument('-p', '--password', default='', help='Set a password (No password by default)')
    parser.add_argument('-t', '--type', default='mp4', choices=['mp4', 'mkv'], help='Set the output file type (The default is mp4)')
    parser.add_argument('-c', '--cover', default=None, help='Specify the shell for MP4 video（By default, the search is under the same path of the program）')
    parser.add_argument('-r', '--reveal', action='store_true', help='Perform the de-steganography')
    parser.add_argument('-pf', '--password-file', default=None, help='Specify the password file path')
    ...
    if args.input:
        ...
        if args.cover is None:
            ...
            if mp4list:
                ...
                print('Please specify the shell for the MP4 file')
                exit(1)  # 退出程序
